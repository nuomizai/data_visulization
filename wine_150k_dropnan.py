...
for index, row in data.iterrows():
    if type(row['country']) == float and math.isnan(row['country']) \
            ...
            or type(row['price']) == float and math.isnan(row['price']) \
            or type(row['points']) == float and math.isnan(row['points']):
        index_list.append(index)
data = data.drop(index_list)
data.to_csv(path + "wine_150k_v2.csv", index=True, sep=',')
