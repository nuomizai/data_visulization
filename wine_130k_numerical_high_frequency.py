...
data = pd.read_csv('wine-reviews/winemag-data-130k-v2.csv', encoding='utf-8',
                   usecols=['points', 'price'])

points = data['points'].value_counts(sort=True).index[0]
price = data['price'].value_counts(sort=True).index[0]
print(points, price)
for index, row in data.iterrows():
    if math.isnan(row['points']):
        row['points'] = points
...
path = 'fig_130k_numerical/high_frequency/'
